...
module.exports = {
  ...
  preprocessReplacements: function(replacements) {
    //TODO: Array.flatMap support
    var flatMap = function(arr, mapper){
      var result = [];
      for(var i = 0; i< arr.length; i++){
        var mapped = mapper(arr[i]);
    ...
    var processGenerator = function(generator) {
      return flatMap(generator.replacements, function(r) {
        return generator.patterns.map(function(p) {
          return Object.keys(p).reduce(function(prev, curr){
            return Object.assign({}, prev, {
              [curr]: Object.keys(r).reduce(function(prev2, curr2, {}){
                  return prev2.replace(new RegExp(curr2, 'g'), r[curr2]);
              }, p[curr])
            });
          }, {});
        })
    ...
    var process = function(replacements){
      return flatMap(replacements, function(r) {
        switch(r.type){
          case "generator":
            return process(processGenerator(r));
          default:
            return r;
        }
      });
    ...
    var result = process(replacements);
    return result;
  }
}